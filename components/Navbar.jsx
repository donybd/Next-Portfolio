...
const NavItems = ({ isNavOpen, setIsNavOpen }) => {
  ...
  return (
    <>
      <motion.div className={`fixed z-[45] w-full h-screen flex items-center justify-center backdrop-blur-sm transition-all ease duration-700 overflow-hidden`} variants={navVariant} animate={isNavOpen ? "open" : "closed"} initial={false}>
...
const Navbar = () => {
  ...
  return (
    <>
      ...
      >
        <div>
          <h1 className={`text-2xl ml-2 md:ml-0 transition-colors ease duration-500 ${isNavOpen ? "text-white" : ""}`}>Donybd</h1>
        </div>
